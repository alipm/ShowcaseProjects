...
public class ArchiveAdapter extends RecyclerView.Adapter<ArchiveAdapter.ArchiveViewHolder> {
    ...
    private final ArrayList<MailArchive> mailArchives;
    private int i10dp;
    private int i20dp;
    private int i40dp;
    private int i3dp;
    private int i5dp;
    private int i15dp;
    int[] imgResources = new int[]{R.drawable.profile_0,R.drawable.profile_1, R.drawable.profile_2, R.drawable.profile_3, R.drawable.profile_4};
    View selectedItem;

    public ArchiveAdapter(Context clContext, ArrayList<MailArchive> mailArchives)
    {
        this.clContext = clContext;
        this.mailArchives = mailArchives;
    }
    ...
    {
        MailArchive mailArchive = mailArchives.get(i);
        archiveViewHolder.tvUserName.setText(mailArchive.getsSenderName());
        archiveViewHolder.tvUserRole.setText(mailArchive.getsUserRole());
        archiveViewHolder.tvTimeAgo.setText(mailArchive.getsTimeAgo());
        archiveViewHolder.tvDescription.setText(mailArchive.getsDescription());
        archiveViewHolder.ivSenderImg.setImageResource(imgResources[i % 5]);
        if (mailArchive.isFavorite())
    ...
    private LinearLayout getRightSideView()
    {
        ...
        STextView tvUserName = new STextView(clContext);
        ...
        STextView tvTimeAgo = new STextView(clContext);
        ...
        STextView tvUserRole = new STextView(clContext);
        ...
        STextView tvDescription = new STextView(clContext);
    ...
    class ArchiveViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View view)
        {
            if (view.getId() == R.id.iv_favortie)
            {
                ...
                //                if ()
                mailArchives.get(getAdapterPosition()).setFavorite(!mailArchives.get(getAdapterPosition()).isFavorite());
                if (mailArchives.get(getAdapterPosition()).isFavorite())
